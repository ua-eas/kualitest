 ...
 * @author rbtucker
 */
public class RemoveParameterNamesDlg extends BaseSetupDlg {
     ...
     * @param mainFrame
     */
    public RemoveParameterNamesDlg(TestCreator mainFrame) {
        super(mainFrame);
        setTitle("Parameters to remove from requests");
        initComponents();
    }

    private void initComponents() {
        tp = new TablePanel(buildNameTable());
        tp.addAddButton(this, Constants.ADD_NAME_ACTION, "add remove parameter name");
        tp.addDeleteButton(this, Constants.REMOVE_NAME_ACTION, "remove selected parameter name");
    ...
    private BaseTable buildNameTable() {
        ...
        config.setTableName("remove-parameter-names");
        ...
        if (getConfiguration().getParametersToRemove() != null) {
            config.setData(new ArrayList<String>(Arrays.asList(getConfiguration().getParametersToRemove().getNameArray())));
        }
    ...
    protected void handleOtherActions(String actionCommand) {
        if (Constants.REMOVE_NAME_ACTION.equals(actionCommand)) {
            ...
            if ((l != null) && !l.isEmpty()) {
                ...
                if (UIUtils.promptForDelete(RemoveParameterNamesDlg.this, 
